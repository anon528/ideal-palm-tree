...
for outer in tqdm(range(1, 7), position=0):
    ...
    if redundancy != 'redundant':
        df_train = pd.merge(df_train, non_redundand)
    ...
    # Store models (comment out if not needed):
    with open('models/ranker_' + str(outer) + '.pkl','wb') as f:
        pkl.dump(ranker, f)

final_df = pd.concat(df_test_list)[['qid', 'pdb_code', 'filename', 'peptide', 'allele', 'Score']]
final_df.to_csv("./res_v5/LKPO_" + objective + '_' + res_file)
...
for outer in tqdm(range(1, 7), position=0):
    ...
    y_train = df_train[RMSD_type].values
    df_train = df_train.drop(columns=column_drop)
    df_test = df[~df["allele"].isin(outer_split_pdb_list)]
    #df_test = pd.merge(df_test, non_redundand)
    y_test = df_test[RMSD_type].values
    df_test_ranker = df_test.drop(columns=column_drop)
    if objective == 'regr':
    ...
    df_test_list.append(df_test)
final_df = pd.concat(df_test_list)[['qid', 'pdb_code', 'filename', 'peptide', 'allele', 'Score']]
final_df.to_csv("./res_v5/LKAO_" + objective + '_' + res_file)
...
for outer in tqdm(range(1, 7), position=0):
    ...
    y_train = df_train[RMSD_type].values
    df_train = df_train.drop(columns=column_drop)
    df_test = df[~df["pep_len"].isin(outer_split_pdb_list)]
    #df_test = pd.merge(df_test, non_redundand)
    y_test = df_test[RMSD_type].values
    df_test_ranker = df_test.drop(columns=column_drop)
    if objective == 'regr':
    ...
    df_test_list.append(df_test)
final_df = pd.concat(df_test_list)[['qid', 'pdb_code', 'filename', 'peptide', 'allele', 'Score']]
final_df.to_csv("./res_v5/LOLO_" + objective + '_' + res_file)
